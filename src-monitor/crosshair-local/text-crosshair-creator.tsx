...
export default function TextCrosshairCreator({ crosshairs }: { crosshairs: string[] }) {
  const $ = useLocale();
  ...
  const onConfirm = async () => {
    await invoke("create_text_crosshair", { path: `${crosshair_dir}/${name}.txt`, content: url });
    close();
  };
  const unlistenFnRef = useRef<Function | null>(null);
  useEffect(() => {
    listen("create_text_crosshair_clicked", () => {
      setName("");
      setUrl("");
      setHelpTextUrl("");
      setHelpTextName("");
  ...
  return (
    <Dialog open={open} onClose={close} fullWidth>
      <DialogTitle display="flex" alignItems="center" gap="0.5rem">
        ...
        <Tip>{$("A .txt file target a image URL")}</Tip>
      </DialogTitle>
      <DialogContent>
