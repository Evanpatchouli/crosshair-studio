...
双击打开程序，即可在桌面上显示一个透明的准星。程序默认内置了 12 种 `+` 符号作为准星图案，可自行切换。

程序启动后，默认将准星置于顶层，并默认鼠标穿透，即准星图片不会遮挡其他窗口的点击事件。

- 准星目录：程序已安装目录下的 `crosshairs` 文件夹为准星目录，可自行添加透明背景的准星图片（`*.svg`, `*.png`, `*.jpg`, `*.jpeg`, `*.gif`）
...
- 全局快捷键：（系统级快捷键）
  - 切换准星：`Ctrl + Alt + Q`，可设置
  - 切换默认准星：`Ctrl + Alt + D`，可设置
  - 设为默认准星：`Ctrl + Alt + S`，可设置
  - 置顶 开/关：`Ctrl + Alt + P`，可设置
  - 穿透 开/关：默认关闭，可设置
  - 打开控制台：`Ctrl + Alt + C`，可设置
  - 重启应用：`Ctrl + Alt + R`，可设置
  - 退出应用：`Ctrl + Alt + E`，可设置
- 应用快捷键：（应用级快捷键）
  - 切换准星：`Q`
...
Crosshair Studio 软件本体及其资源文件均位于安装目录下，仅有大小约 5kb 的数据文件存储在用户目录下的 `AppData\Roaming\com.crosshair-studio.app` 文件夹中。
